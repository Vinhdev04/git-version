...
**fork**: Tạo một bản sao của repo trên GitHub
- 👉 Dùng khi:
    + muốn làm việc trên một repo mà không muốn thay đổi repo gốc (của người khác)
    + muốn tạo một repo mới từ một repo khác
```
    Fork → Clone → Code → Push → Pull Request
    - Cần add remote mớimới fork vào để git push được vào repo gốc (của người khác)
        `git remote add <name_remote_fork> <url_repo_fork>`
```

-   🧠 Cách hoạt động
    -   git fork sẽ tạo một bản sao của repo trên GitHub
    -   bạn có thể làm việc trên bản sao mà không ảnh hưởng đến repo gốc (của người khác)
    -   bạn có thể merge bản sao vào repo gốc khi bạn đã hoàn thành việc làm việc
⚠️ Lưu ý
    - Fork không tự động cập nhật theo repo gốc
    - → cần sync nếu repo gốc có thay đổi
    - Dùng khi làm open-source hoặc làm việc với repo người khác
---
**git rebase**
-   💡 Vậy rebase dùng để làm gì?
    1. 🧹 Làm sạch lịch sử commit
    2.  bỏ “rác commit”
    3.   tránh merge commit không cần thiết
    4. 🧑‍💻 Chuẩn workflow team: `feature branch → rebase → PR → merge`
- Ví dụ:
    - merge thông thường:
        ```
            main:    A --- B --- E
            ...
            main: A---B---E------M
                           \    /
                            C--D
            👉 Lịch sử bị “rẽ nhánh + merge commit” nhìn rối
        ```
    - rebase: **git checkout feature, git rebase main**
        ```
            ...
            👉 Lịch sử thẳng, dễ đọc
        ```
- ⚠️ Điểm quan trọng nhất
    - ❌ Rebase KHÔNG phải để đưa code vào main
    - ✔️ Rebase chỉ chuẩn bị code để merge vào main
- 🧠 Tóm gọn cực dễ nhớ
    - **rebase** → “làm đẹp lịch sử, thay dổi commit trước khi nhập main”
    - **merge**→ “gộp code vào main”, dữ nguyên commitcommit
---
**🍒 git cherry-pick là gì?**
- git cherry-pick dùng để lấy 1 commit cụ thể từ branch khác và áp vào branch hiện tại.

- 👉 Nói đơn giản:  `“Không cần merge cả nhánh, chỉ lấy đúng 1 commit mình cần”   `
- Ví dụ: **git cherry-pick <commit_hash>**
    ```
        main:    A --- B --- E
                      \
        feature:       C --- D
    ...
    ```
        main: A --- B --- E --- D'
    ```
    - `⚠️ Lưu ý: D' là commit mới (khác hash với D)`
- 💡 Khi nào dùng cherry-pick?
- ✔️ Rất hữu ích khi:
        Fix bug ở branch khác nhưng cần gấp
        Lấy 1 feature nhỏ từ branch khác
        Không muốn merge cả branch lớn
---